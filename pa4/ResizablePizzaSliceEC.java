 ...
 *              Moreover, while user drag lines arround canvas, all pizza
 *              will always point to the intersection of two lines.
 */
 ...
 *              change since they have new location. Also, when user drag
 *              lines, then angle of pizza will change accordingly, and 
 *              always point to intersection of two lines.
 */
public class ResizablePizzaSliceEC extends ActiveObject {
  ...
  // size of pizza
  private double size;
  private static final double DOUBLE_SIZEL = 180;
  private static final double HALF_SIZEL  = 45;

  //angle variable
  private static final double ANGLE1 = 180;
  private static final double ANGLE2 = 360;
  ...
  public void checkQuad() {
    ...
    // variables for triangle's side
    double oppositeSide;
    double adjacentSide;

    //angle between horizontal line and line with tip and intersection
    double newAngle;   
 
    // y distance between tip and horizontal line
    oppositeSide = Math.abs( hLine.getStart().getY() - tip.getY());

    // x distance between tip and vertical line
    adjacentSide = Math.abs( vLine.getStart().getX()- tip.getX());

    // calculate angle between horizontal line and line pass tip and
    // intersection
    newAngle = Math.toDegrees( Math.atan2 ( oppositeSide,
      adjacentSide));
    ...
    if ( tip.getX() > vLine.getStart().getX() &&
      ...
      // reset starting angle for pizza
      pizzaSlice.setStartAngle(newAngle / HALF_SIZE);
    }
    ...
    // Check whether pizza on second quadrant
      ...
      //reset starting angle for pizza
      pizzaSlice.setStartAngle( ANGLE1 - newAngle/ HALF_SIZE
          - ARC_ANGLE);
    }
    ...
    // Check whether pizza on fourth quadrant
      ...
      // reset starting angle for pizza
      pizzaSlice.setStartAngle( ANGLE2 - newAngle/HALF_SIZE -
          ARC_ANGLE );
    }
  }
